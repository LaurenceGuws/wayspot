//! NotificationHistoryProvider owns picker rows loaded from the persisted notification cache.

const std = @import("std");
const builtin = @import("builtin");
const history_cache = @import("../notifications/history_cache.zig");
const preview = @import("../notifications/preview.zig");
const search = @import("../search/mod.zig");

const action_prefix = "notification-history:";
const max_action_bytes: u32 = 64;

comptime {
    std.debug.assert(max_action_bytes > action_prefix.len);
}

pub const NotificationHistoryProvider = struct {
    cache: ?history_cache.Cache = null,
    owned_strings: std.ArrayListUnmanaged([]u8) = .empty,

    pub fn deinit(self: *NotificationHistoryProvider, allocator: std.mem.Allocator) void {
    ...
    pub fn collect(
        self: *NotificationHistoryProvider,
    ...
    fn appendRow(
        ...
        const action = try std.fmt.allocPrint(allocator, "{s}{d}:{d}", .{ action_prefix, newest_rank, row.id });
        errdefer allocator.free(action);
        if (action.len > max_action_bytes) return error.NotificationHistoryActionTooLong;
        ...
        try out.append(allocator, search.Candidate.init(.notification, owned_title, owned_subtitle, action));
        self.owned_strings.appendAssumeCapacity(action);
...
test "notification history provider exposes newest cached rows first" {
    ...
    var provider = NotificationHistoryProvider{};
    defer provider.deinit(std.testing.allocator);
    var list = search.CandidateList.empty;
    defer list.deinit(std.testing.allocator);

    try provider.collectAtPathForTest(std.testing.allocator, &list, path, now);
    try std.testing.expectEqual(@as(u32, 2), @as(u32, @intCast(list.items.len)));
    try std.testing.expectEqual(search.CandidateKind.notification, list.items[0].kind);
    try std.testing.expectEqualStrings("new", list.items[0].title);
    try std.testing.expect(std.mem.startsWith(u8, list.items[0].action, action_prefix));
}
