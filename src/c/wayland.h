...
struct wayspot_layer_output { struct wl_output *output; char name[WAYSPOT_LAYER_MAX_OUTPUT_NAME]; uint32_t name_len; };
struct wayspot_layer_globals { struct wl_display *display; struct wl_registry *registry; struct wl_compositor *compositor; struct wl_shm *shm; void *layer_shell; struct wayspot_layer_output outputs[WAYSPOT_LAYER_MAX_OUTPUTS]; uint32_t output_count; };
struct wayspot_layer_configure_state { uint32_t configured; uint32_t closed; uint32_t serial; uint32_t width; uint32_t height; };
struct wayspot_shm_buffer { struct wl_buffer *buffer; void *data; uint32_t byte_len; };

enum wayspot_layer_result { WAYSPOT_LAYER_OK = 0, WAYSPOT_LAYER_INVALID_SIZE = 1, WAYSPOT_LAYER_MEMFD_FAILED = 2, WAYSPOT_LAYER_TRUNCATE_FAILED = 3, WAYSPOT_LAYER_MMAP_FAILED = 4, WAYSPOT_LAYER_SHM_POOL_FAILED = 5, WAYSPOT_LAYER_WL_BUFFER_FAILED = 6, WAYSPOT_LAYER_REGISTRY_FAILED = 7, WAYSPOT_LAYER_REGISTRY_LISTENER_FAILED = 8, WAYSPOT_LAYER_DISPLAY_ROUNDTRIP_FAILED = 9, WAYSPOT_LAYER_SHELL_MISSING = 10, WAYSPOT_LAYER_COMPOSITOR_MISSING = 11, WAYSPOT_LAYER_SHM_MISSING = 12, WAYSPOT_LAYER_INPUT_REGION_FAILED = 13 };
