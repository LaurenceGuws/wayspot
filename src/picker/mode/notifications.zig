//! Notification mode owns notification history and daemon lifecycle row facts.

const std = @import("std");
const candidate = @import("picker_candidate");

pub const restart_open = "daemon:notifications:restart";
...
pub fn collect(allocator: std.mem.Allocator, out: *candidate.Candidate.List) !void {
    try out.append(allocator, candidate.Candidate.init(.mode, "/notifications", "Daemon mode", "/notifications"));
    try out.append(allocator, candidate.Candidate.init(.daemon, "Restart notifications daemon", "Runtime", restart_open));
    try out.append(allocator, candidate.Candidate.init(.mode, "Notification history", "Runtime", history_open));
}
