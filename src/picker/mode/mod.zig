...
/// resolveDaemonCommand returns the command associated with lifecycle rows.
pub fn resolveDaemonCommand(open: []const u8) ?[]const u8 {
...
test "mode exposes slash rows and daemon restart commands" {
    ...
    try std.testing.expectEqual(candidate.Candidate.Kind.daemon, list.items[1].kind);
    ...
    try std.testing.expect(resolveDaemonCommand(list.items[1].open) != null);
}
