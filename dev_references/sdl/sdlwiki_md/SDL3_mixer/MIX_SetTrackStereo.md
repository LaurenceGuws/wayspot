...
| const [MIX_StereoGains](MIX_StereoGains.html) \* | **gains** | the per-channel gains, or NULL to disable spatialization. |
...
This will cause the output of the track to convert to stereo, and then
mix it only onto the Front Left and Front Right speakers, regardless of
the speaker configuration. The left and right channels are modulated by
...
Negative gains are clamped to zero; there is no clamp for maximum, so
one could set the value \> 1.0f to make a channel louder.

The track's 3D position, reported by
[MIX_GetTrack3DPosition](MIX_GetTrack3DPosition.html)(), will be reset
to (0, 0, 0).
