...
Set a [MIX_Track](MIX_Track.html)'s input to an SDL_IOStream providing
raw PCM data.
...
Also, an [MIX_SetTrackAudioStream](MIX_SetTrackAudioStream.html)() can
*also* provide raw PCM audio to a track, via an SDL_AudioStream, which
might be preferable unless the data is already coming directly from an
SDL_IOStream.
