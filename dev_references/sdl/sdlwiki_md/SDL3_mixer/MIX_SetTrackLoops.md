...
|  |  |  |
|----|----|----|
| [MIX_Track](MIX_Track.html) \* | **track** | the track to configure. |
...
This replaces any previously-set remaining loops. A value of 1 will loop
to the start of playback one time. Zero will not loop at all. A value of
-1 requests infinite loops. If the input is not seekable and `num_loops`
isn't zero, this function will report success but the track will stop at
the point it should loop.
...
[MIX_PROP_PLAY_LOOPS_NUMBER](MIX_PROP_PLAY_LOOPS_NUMBER.html). This
