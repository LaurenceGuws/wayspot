...
|                                |           |                      |
|--------------------------------|-----------|----------------------|
| [MIX_Track](MIX_Track.html) \* | **track** | the track to resume. |
...
stopped). Unless the track is currently paused, resuming does nothing,
and returns true. A false return is only used to signal errors here
(such as [MIX_Init](MIX_Init.html) not being called or `track` being
NULL).
