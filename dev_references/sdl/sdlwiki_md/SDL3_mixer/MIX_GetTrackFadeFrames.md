...
(Sint64) Returns less than 0 if the track is fading out, greater than 0
if fading in, zero otherwise.

## Remarks

This specifically checks if the track is *not stopped* (paused or
playing), and it is fading in or out, and returns the number of frames
remaining in the fade.

If fading out, the returned value will be negative. When fading in, the
returned value will be positive. If not fading, this function returns
zero.

On various errors ([MIX_Init](MIX_Init.html)() was not called, the track
is NULL), this returns 0, but there is no mechanism to distinguish
errors from tracks that aren't fading.
