...
point size becomes the index of choosing which size. If the value is too
high, the last indexed size will be the default.

If `closeio` is true, `src` will be automatically closed once the font
is closed. Otherwise you should keep `src` open until the font is
closed.
