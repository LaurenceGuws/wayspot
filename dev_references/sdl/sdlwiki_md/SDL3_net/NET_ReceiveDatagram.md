...
|  |  |  |
|----|----|----|
| [NET_DatagramSocket](NET_DatagramSocket.html) \* | **sock** | the datagram socket to send data through. |
| [NET_Datagram](NET_Datagram.html) \*\* | **dgram** | a pointer to the datagram packet pointer. |
...
This call never blocks; if no new data isn't available at the time of
the call, it returns true immediately. The caller can try again later.
...
[NET_DestroyDatagram](NET_DestroyDatagram.html) when you are done with
them. If you want to save the sender's address past this time, it is
safe to call [NET_RefAddress](NET_RefAddress.html)() on the address and
hold onto the pointer, so long as you call
[NET_UnrefAddress](NET_UnrefAddress.html)() on it when you are done with
it.

Since datagrams can arrive from any address or port on the network
without prior warning, this information is available in the
[NET_Datagram](NET_Datagram.html) object that is provided by this
...
If there's a fatal error, this function will return false. Datagram
sockets generally won't report failures, because there is no state like
a "connection" to fail at this level, but may report failure for
