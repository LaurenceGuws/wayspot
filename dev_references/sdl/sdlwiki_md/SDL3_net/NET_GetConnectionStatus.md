...
|  |  |  |
|----|----|----|
| [NET_StreamSocket](NET_StreamSocket.html) \* | **sock** | the stream socket to query. |
...
failed, [NET_WAITING](NET_WAITING.html) if still connecting; if
[NET_FAILURE](NET_FAILURE.html), call SDL_GetError() for details.

## Remarks

The [NET_StreamSocket](NET_StreamSocket.html) objects returned by
[NET_CreateClient](NET_CreateClient.html) take time to do negotiate a
connection to a server, so it does so *asynchronously* instead of making
...
Note that if the connection succeeds, but later the connection is
dropped, this will still report the connection as successful, as it only
deals with the initial asynchronous work of getting connected; you'll
know the connection dropped later when your reads and writes report
failures.
