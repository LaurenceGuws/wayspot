...
|  |  |  |
|----|----|----|
| [NET_DatagramSocket](NET_DatagramSocket.html) \* | **sock** | The socket to set a failure rate on. |
...
The library will randomly lose packets (both incoming and outgoing) at
an average matching `percent_loss`. Setting this to zero (the default)
will disable the simulation. Setting to 100 means *everything* fails
...
This is intended for debugging purposes, to simulate real-world
conditions that are various degrees of terrible. You probably should
*not* call this in production code, where you'll likely see real
failures anyhow.
