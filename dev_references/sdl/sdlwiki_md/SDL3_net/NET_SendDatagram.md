...
| [NET_Address](NET_Address.html) \* | **address** | the [NET_Address](NET_Address.html) object address. |
...
Datagram packets might arrive in a different order than you sent them,
or they may just be lost while travelling across the network. You have
to plan for this.
...
General wisdom is that you shouldn't send a packet larger than 1500
bytes over the Internet, as bad routers might fragment or lose larger
ones, but this limit is not hardcoded into SDL_net and in good
conditions you might be able to send significantly more.
...
what is still queued, as datagram transmission is unreliable, so you
should never assume anything about queued data.
