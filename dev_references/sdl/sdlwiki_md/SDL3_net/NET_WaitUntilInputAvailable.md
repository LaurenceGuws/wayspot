...
become available on any of them. The calling thread is put to sleep
until such a time.

The following things can be specified in the `vsockets` array, cast to
`void *`:

- [NET_Server](NET_Server.html) (reports new input when a connection is
  ready to be accepted with [NET_AcceptClient](NET_AcceptClient.html)())
- [NET_StreamSocket](NET_StreamSocket.html) (reports new input when the
  remote end has sent more bytes of data to be read with
  [NET_ReadFromStreamSocket](NET_ReadFromStreamSocket.html), or if the
  socket finished making its initial connection).
- [NET_DatagramSocket](NET_DatagramSocket.html) (reports new input when
  a new packet arrives that can be read with
...
instructs the library to wait indefinitely, and a timeout of 0 just
checks the current status and returns immediately.

This returns the number of items that have new input, but it does not
tell you which ones; since access to them is non-blocking, you can just
try to read from each of them and see which are ready. If nothing is
ready and the timeout is reached, this returns zero. On error, this
returns -1.
