...
|  |  |  |
|----|----|----|
| [NET_StreamSocket](NET_StreamSocket.html) \* | **sock** | the stream socket to receive data from. |
...
Stream sockets are *reliable*, which means data sent over them will
...
might arrive in any order, or not arrive at all, but you never wait for
missing chunks to show up.)

Stream sockets are *bidirectional*; you can read and write from the same
...
remote side. This function may return less data than requested,
...
the call, it returns 0 immediately. The caller can try again later.

If the connection has failed (remote side dropped us, or one of a
million other networking failures occurred), this function will report
failure by returning -1. Stream sockets only report failure for
unrecoverable conditions; once a stream socket fails, you should assume
it is no longer usable and should destroy it with
