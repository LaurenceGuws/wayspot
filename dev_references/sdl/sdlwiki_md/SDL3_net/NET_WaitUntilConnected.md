...
|  |  |  |
|----|----|----|
| [NET_StreamSocket](NET_StreamSocket.html) \* | **sock** | The [NET_StreamSocket](NET_StreamSocket.html) object to wait on. |
...
if successfully connected, [NET_FAILURE](NET_FAILURE.html) if connection
failed, [NET_WAITING](NET_WAITING.html) if still connecting (this
function timed out without resolution); if
[NET_FAILURE](NET_FAILURE.html), call SDL_GetError() for details.

## Remarks

The [NET_StreamSocket](NET_StreamSocket.html) objects returned by
[NET_CreateClient](NET_CreateClient.html) take time to do their work, so
it does so *asynchronously* instead of making your program wait an
...
checks the current status and returns immediately (and is functionally
...
[NET_GetConnectionStatus](NET_GetConnectionStatus.html)() from time to
...
different threads may access different socket at the same time without
problems.
