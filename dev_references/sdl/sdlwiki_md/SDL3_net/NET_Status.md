...
exception of functions named "Wait", which are intended to do nothing
but block until a task completes. Functions that are attempting to do
something that might block, or are querying the status of a task
in-progress, will return a [NET_Status](NET_Status.html), so an app can
see if a task completed, and its final outcome.
