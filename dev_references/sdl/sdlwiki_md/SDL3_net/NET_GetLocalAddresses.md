...
([NET_Address](NET_Address.html) \*\*) Returns A NULL-terminated array
of [NET_Address](NET_Address.html) pointers, one for each bindable
address on the system, or NULL on error; call SDL_GetError() for
details.
...
You almost never need this function; first, it's hard to tell *what* is
a good address to bind to, without asking the user (who will likely find
it equally hard to decide). Second, most machines will have lots of
*private* addresses that are accessible on the same LAN, but not public
ones that are accessible from the outside Internet.

Usually it's better to use [NET_CreateServer](NET_CreateServer.html)()
or [NET_CreateDatagramSocket](NET_CreateDatagramSocket.html)() with a
NULL address, to say "bind to all interfaces."
