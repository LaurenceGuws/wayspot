...
create a GPU texture with the format
SDL_GPU_TEXTUREFORMAT_R8G8B8A8_UNORM with no mip levels. It can be bound
as a sampled texture from a graphics or compute pipeline and as a a
readonly storage texture in a compute pipeline.
...
deal with SDL_IOStream:
\`[IMG_LoadGPUTexture](IMG_LoadGPUTexture.html)(device, copy_pass,
"filename.jpg", width, height) will call this function and manage those
details for you, determining the file type from the filename's
extension.

There is also [IMG_LoadGPUTexture_IO](IMG_LoadGPUTexture_IO.html)(),
which is equivalent to this function except that it will rely on
SDL_image to determine what type of data it is loading, much like
passing a NULL for type.
