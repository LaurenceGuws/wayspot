...
(unsigned long) Returns the parsed `unsigned long`, or 0 if no number
could be parsed.
...
If the parsed number does not fit inside an `unsigned long`, the result
is clamped to the maximum representable `unsigned long` value.
