...
|  |  |  |
|----|----|----|
| const [SDL_PixelFormatDetails](SDL_PixelFormatDetails.html) \* | **format** | a pointer to [SDL_PixelFormatDetails](SDL_PixelFormatDetails.html) describing the pixel format. |
| const [SDL_Palette](SDL_Palette.html) \* | **palette** | an optional palette for indexed formats, may be NULL. |
...
([Uint32](Uint32.html)) Returns a pixel value.
...
a 16-bpp format the return value can be assigned to a
[Uint16](Uint16.html), and similarly a Uint8 for an 8-bpp format).
