...
(int) Returns the actual number of bytes written and -1 on on failure;
...
to [SDL_hid_write](SDL_hid_write.html)() will always contain one more
byte than the report contains. For example, if a hid report is 16 bytes
long, 17 bytes must be passed to [SDL_hid_write](SDL_hid_write.html)(),
...
[SDL_hid_write](SDL_hid_write.html)() will send the data on the first
OUT endpoint, if one exists. If it does not, it will send the data
