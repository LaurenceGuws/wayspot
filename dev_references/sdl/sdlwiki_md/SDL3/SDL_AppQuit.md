...
| [SDL_AppResult](SDL_AppResult.html) | **result** | the result code that terminated the app (success or failure). |
...
[SDL_AppInit](SDL_AppInit.html) requests termination at startup.
...
You do not need to call [SDL_Quit](SDL_Quit.html)() in this function, as
SDL will call it after this function returns and before the process
terminates, but it is safe to do so.
...
pointer, this will be NULL. This function call is the last time this
pointer will be provided, so any resources to it should be cleaned up
here.
...
[SDL_AppEvent](SDL_AppEvent.html)() may get called concurrently with
this function if other threads that push events are still active.
