...
|  |  |  |
|----|----|----|
| const [SDL_Event](SDL_Event.html) \* | **event** | an event to describe. May be NULL. |
...
(int) Returns number of bytes needed for the full string, not counting
...
[SDL_snprintf](SDL_snprintf.html)(): `buf` will always be
NULL-terminated (unless `buflen` is zero), and will be truncated if
`buflen` is too small. The return code is the number of bytes needed for
the complete string, not counting the NULL-terminator, whether the
string was truncated or not. Unlike [SDL_snprintf](SDL_snprintf.html)(),
though, this function never returns -1.
