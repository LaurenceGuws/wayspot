...
Wait until the specified timeout (in milliseconds) for the next
available event.
...
| [Sint32](Sint32.html) | **timeoutMS** | the maximum number of milliseconds to wait for the next available event. |
...
The timeout is not guaranteed, the actual wait time could be longer due
