...
|  |  |  |
|----|----|----|
| [SDL_IOStream](SDL_IOStream.html) \* | **src** | the data source for the WAVE data. |
...
If this function succeeds, the return value is zero and the pointer to
the audio data allocated by the function is written to `audio_buf` and
its length in bytes to `audio_len`. The
[SDL_AudioSpec](SDL_AudioSpec.html) members `freq`, `channels`, and
`format` are set to the values of the audio data in the buffer.

It's necessary to use [SDL_free](SDL_free.html)() to free the audio data
...
the RIFF chunk. The hints
[`SDL_HINT_WAVE_RIFF_CHUNK_SIZE`](SDL_HINT_WAVE_RIFF_CHUNK_SIZE.html),
[`SDL_HINT_WAVE_TRUNCATION`](SDL_HINT_WAVE_TRUNCATION.html), and
[`SDL_HINT_WAVE_FACT_CHUNK`](SDL_HINT_WAVE_FACT_CHUNK.html) can be used
to tune the behavior of the loading process.
...
Note that the [SDL_LoadWAV](SDL_LoadWAV.html) function does this same
thing for you, but in a less messy way:
