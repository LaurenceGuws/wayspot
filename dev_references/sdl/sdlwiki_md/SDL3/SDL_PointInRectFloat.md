...
|                                        |       |                        |
|----------------------------------------|-------|------------------------|
| const [SDL_FPoint](SDL_FPoint.html) \* | **p** | the point to test.     |
| const [SDL_FRect](SDL_FRect.html) \*   | **r** | the rectangle to test. |
...
NULL, and `p`'s x and y coordinates are \>= to the rectangle's top left
corner, and \<= the rectangle's x+w and y+h. So a 1x1 rectangle
considers point (0,0) and (0,1) as "inside" and (0,2) as not.
