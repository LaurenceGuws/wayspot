...
|  |  |  |
|----|----|----|
| [SDL_CursorFrameInfo](SDL_CursorFrameInfo.html) \* | **frames** | an array of cursor images composing the animation. |
...
([SDL_Cursor](SDL_Cursor.html) \*) Returns the new cursor on success or
...
as surfaces and durations in an array of
[SDL_CursorFrameInfo](SDL_CursorFrameInfo.html) structs. The hot spot
coordinates are universal to all frames, and all frames must have the
same dimensions.

Frame durations are specified in milliseconds. A duration of 0 implies
an infinite frame time, and the animation will stop on that frame. To
create a one-shot animation, set the duration of the last frame in the
sequence to 0.

If this function is passed surfaces with alternate representations added
with
[SDL_AddSurfaceAlternateImage](SDL_AddSurfaceAlternateImage.html)(), the
surfaces will be interpreted as the content to be used for 100% display
scale, and the alternate representations will be used for high DPI
situations. For example, if the original surfaces are 32x32, then on a
2x macOS display or 200% display scale on Windows, a 64x64 version of
the image will be used, if available. If a matching version of the image
...
If the underlying platform does not support animated cursors, this
function will fall back to creating a static color cursor using the
