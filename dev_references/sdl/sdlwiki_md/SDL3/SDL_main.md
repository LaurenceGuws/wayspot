...
before `main` runs, or the platform doesn't actually *use* a function
called "main", SDL will do some macro magic to redefine `main` to
[`SDL_main`](SDL_main.html) and provide its own `main`.

Apps should include `SDL_main.h` in the same file as their `main`
function, and they should not use that symbol for anything else in that
file, as it might get redefined.
...
[README-main-functions](README-main-functions.html), (or
