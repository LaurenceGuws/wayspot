...
[SDL_wcslen](SDL_wcslen.html)(dst) - 1 wide characters from `src` to the
end of the wide string in `dst`, then appends a null terminator.

`src` and `dst` must not overlap.

If `maxlen` - [SDL_wcslen](SDL_wcslen.html)(dst) - 1 is less than or
equal to 0, then `dst` is unmodified.
