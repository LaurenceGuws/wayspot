...
| [SDL_GPUTextureFormat](SDL_GPUTextureFormat.html) | **format** | a supported format for the OpenXR swapchain. |
| XrSwapchain \* | **swapchain** | a pointer filled in with the created OpenXR swapchain. |
| [SDL_GPUTexture](SDL_GPUTexture.html) \*\*\* | **textures** | a pointer filled in with the array of created swapchain images. |
...
Applications are still allowed to call `xrEnumerateSwapchainImages` on
the returned XrSwapchain if they need to get the exact size of the
array.
