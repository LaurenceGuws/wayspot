...
| [Uint32](Uint32.html) | **allowed_frames_in_flight** | the maximum number of frames that can be pending on the GPU. |
...
you have submitted 2 frames for presentation, if the GPU has not
finished working on the first frame,
[SDL_AcquireGPUSwapchainTexture](SDL_AcquireGPUSwapchainTexture.html)()
will fill the swapchain texture pointer with NULL, and
...
Note that calling this function will stall and flush the command queue
to prevent synchronization issues.

The minimum value of allowed frames in flight is 1, and the maximum is
3.
