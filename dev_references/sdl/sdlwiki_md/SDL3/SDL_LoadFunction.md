# SDL_LoadFunction

Look up the address of the named function in a shared object.

## Header File

Defined in
[\<SDL3/SDL_loadso.h\>](https://github.com/libsdl-org/SDL/blob/main/include/SDL3/SDL_loadso.h)

## Syntax

<div id="cb1" class="sourceCode">

``` sourceCode
SDL_FunctionPointer SDL_LoadFunction(SDL_SharedObject *handle, const char *name);
```

</div>

## Function Parameters

|  |  |  |
|----|----|----|
| [SDL_SharedObject](SDL_SharedObject.html) \* | **handle** | a valid shared object handle returned by [SDL_LoadObject](SDL_LoadObject.html)(). |
| const char \* | **name** | the name of the function to look up. |

## Return Value

([SDL_FunctionPointer](SDL_FunctionPointer.html)) Returns a pointer to
the function or NULL on failure; call
[SDL_GetError](SDL_GetError.html)() for more information.

## Remarks

This function pointer is no longer valid after calling
[SDL_UnloadObject](SDL_UnloadObject.html)().

This function can only look up C function names. Other languages may
have name mangling and intrinsic language support that varies from
compiler to compiler.

Make sure you declare your function pointers with the same calling
convention as the actual library function. Your code will crash
mysteriously if you do not do this.

If the requested function doesn't exist, NULL is returned.

## Thread Safety

It is safe to call this function from any thread.

## Version

This function is available since SDL 3.2.0.

## Code Examples

<div id="cb2" class="sourceCode">

``` sourceCode
/* Variable declaration */
SDL_SharedObject* myHandle = NULL;
const char* myFunctionName = "myFancyFunction";
void (*myFancyFunction)(int anInt);

/* Dynamically load mylib.so */
myHandle = SDL_LoadObject("mylib.so");

/* Load the exported function from mylib.so
 * The exported function has the following prototype
 * void myFancyFunction(int anInt);
 */
myFancyFunction = (void (*)(int))SDL_LoadFunction(myHandle, myFunctionName);

/* Call myFancyFunction with a random integer */
if (myFancyFunction != NULL) {
    myFancyFunction(15);
} else {
    /* Error handling here */
}
```

</div>

## See Also

- [SDL_LoadObject](SDL_LoadObject.html)

------------------------------------------------------------------------

[CategoryAPI](CategoryAPI.html),
[CategoryAPIFunction](CategoryAPIFunction.html),
[CategorySharedObject](CategorySharedObject.html)
