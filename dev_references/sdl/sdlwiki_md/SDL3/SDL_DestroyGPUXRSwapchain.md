# SDL_DestroyGPUXRSwapchain

Destroys and OpenXR swapchain previously returned by
[SDL_CreateGPUXRSwapchain](SDL_CreateGPUXRSwapchain.html).

## Header File

Defined in
[\<SDL3/SDL_openxr.h\>](https://github.com/libsdl-org/SDL/blob/main/include/SDL3/SDL_openxr.h)

## Syntax

<div id="cb1" class="sourceCode">

``` sourceCode
XrResult SDL_DestroyGPUXRSwapchain(SDL_GPUDevice *device, XrSwapchain swapchain, SDL_GPUTexture **swapchainImages);
```

</div>

## Function Parameters

|  |  |  |
|----|----|----|
| [SDL_GPUDevice](SDL_GPUDevice.html) \* | **device** | a GPU context. |
| XrSwapchain | **swapchain** | a swapchain previously returned by [SDL_CreateGPUXRSwapchain](SDL_CreateGPUXRSwapchain.html). |
| [SDL_GPUTexture](SDL_GPUTexture.html) \*\* | **swapchainImages** | an array of swapchain images returned by the same call to [SDL_CreateGPUXRSwapchain](SDL_CreateGPUXRSwapchain.html). |

## Return Value

(XrResult) Returns the result of the call.

## Version

This function is available since SDL 3.6.0.

## See Also

- [SDL_CreateGPUDeviceWithProperties](SDL_CreateGPUDeviceWithProperties.html)
- [SDL_CreateGPUXRSession](SDL_CreateGPUXRSession.html)
- [SDL_CreateGPUXRSwapchain](SDL_CreateGPUXRSwapchain.html)

------------------------------------------------------------------------

[CategoryAPI](CategoryAPI.html),
[CategoryAPIFunction](CategoryAPIFunction.html),
[CategoryOpenxr](CategoryOpenxr.html)
