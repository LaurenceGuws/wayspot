# SDL_GetTrayEntries

Returns a list of entries in the menu, in order.

## Header File

Defined in
[\<SDL3/SDL_tray.h\>](https://github.com/libsdl-org/SDL/blob/main/include/SDL3/SDL_tray.h)

## Syntax

<div id="cb1" class="sourceCode">

``` sourceCode
const SDL_TrayEntry ** SDL_GetTrayEntries(SDL_TrayMenu *menu, int *count);
```

</div>

## Function Parameters

|  |  |  |
|----|----|----|
| [SDL_TrayMenu](SDL_TrayMenu.html) \* | **menu** | The menu to get entries from. |
| int \* | **count** | An optional pointer to obtain the number of entries in the menu. |

## Return Value

(const [SDL_TrayEntry](SDL_TrayEntry.html) \*\*) Returns a
NULL-terminated list of entries within the given menu. The pointer
becomes invalid when any function that inserts or deletes entries in the
menu is called.

## Thread Safety

This function should be called on the thread that created the tray.

## Version

This function is available since SDL 3.2.0.

## See Also

- [SDL_RemoveTrayEntry](SDL_RemoveTrayEntry.html)
- [SDL_InsertTrayEntryAt](SDL_InsertTrayEntryAt.html)

------------------------------------------------------------------------

[CategoryAPI](CategoryAPI.html),
[CategoryAPIFunction](CategoryAPIFunction.html),
[CategoryTray](CategoryTray.html)
